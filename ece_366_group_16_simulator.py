def simulate(Instruction,Memory):
    ...
    while(not(finished)):
        ...
        if (fetch[1:3] == "101"):  #INIT
            ...
            Reg[R] = Reg[R] + imm
            PC += 1
        elif (fetch[1:7] == "1111100"):    #ADDN
            fetch = fetch.replace("1111100","")
            R = 3
            Reg[R] = Reg[R] - 1
            PC += 1
        elif (fetch[1:3] == "100"):    #ADD
            ...
            PC += 1
        elif (fetch[1:5] == "00000"):   #SUBR0
            fetch = fetch.replace("00000","")
            Ry = int(fetch[0])
            Reg[0] = Reg[0] - Reg[Ry]
            PC += 1
        elif (fetch[1:4] == "0001"):    #XOR
            ...
            if( Reg[Ry] < Reg[0] ):
                ...
                Reg[3] = 0
            PC += 1
        elif (fetch[1:7] == "0001000"):    #CNTR0
            fetch = fetch.replace("0001000","")
            imm = int(fetch[0])
            PC = PC + imm
        elif (fetch[1:3] == "010"):    #JIF
            fetch = fetch.replace("010","")
            imm = int(fetch[0])
            PC = PC + imm
        elif(fetch[1:7] == "0001111"): #HLT
            finished = True
